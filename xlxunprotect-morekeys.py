...
def decrypt_c1_keys(data):
    ...
    if o < len(data):
        print >> sys.stderr, "c1: left: %s" % data[o:].encode("hex")
...
def decodechunk(hdr, data):
    if hdr.version in keys:
        print "enc = ", data.encode('hex')
        print "key = ", keys[hdr.version]['key'].encode('hex'), 'iv=', keys[hdr.version]['iv'].encode('hex')
...
class ProtectParser:
    ...
    def parsetoken(self, text):
        ...
        print >> sys.stderr, "protected: unknown syntax: '%s'" % text
        return 1, None, None
